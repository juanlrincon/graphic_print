 ...
 */
	function select($fieldName, $options = array(), $selected = null, $attributes = array(), $showEmpty = '') {
		...
		$style = null;
		$tag = null;
$attributes['onchange'] = 'setbox(this);';
		$clean_name = str_replace('.', '_', $fieldName);

/*		if (isset($options['1']))
			$clean_name2 = $options['1'];
		else
			$clean_name2 = $options['01'];
*/
	$clean_name2 = reset($options).'';
$attributes['id'] = $clean_name;
		...
		// todo: revisar todos los selects que queden con estos rangos
/*
		if ((strlen($clean_name2)) <= 0)
			$padding = strlen($clean_name2) * 130;
		else if ((strlen($clean_name2)) > 0 and (strlen($clean_name2)) <= 3)
			$padding = strlen($clean_name2) * 40;
		else if ((strlen($clean_name2)) > 3 and (strlen($clean_name2)) <= 6)
			$padding = strlen($clean_name2) * 70;
		else if ((strlen($clean_name2)) > 6 and (strlen($clean_name2)) <= 10)
			$padding = strlen($clean_name2) * 17;		
		else if ((strlen($clean_name2)) > 10)
			$padding = strlen($clean_name2) * 9;
*/
		/*echo '-------------->';
		print_r($select);
		echo '<-----------';
		*/
		$padding = strlen($clean_name2) + 79;
		// modificado aqui por juan 
		//$styling_select1 = '-->'.strlen($clean_name2).$clean_name2.$padding.'<--';
		$styling_select1 = '<div class="selectBox2"><div class="selectBox" style="padding-left:'.$padding.'px;"><div class="box" id="box_'.$clean_name.'">&nbsp;</div>'; 
		$styling_select2 = '</div></div><script>setbox0("'.$clean_name.'");</script>';

		return $styling_select1.$this->output(implode("\n", $select)).$styling_select2;
	}
/**
 ...
 */
	function dateTime($fieldName, $dateFormat = 'DMY', $timeFormat = '12', $selected = null, $attributes = array(), $showEmpty = true) {
		...
		$defaults = array( // juan aqui cambie 'separator' => '-' por 'separator' => ' ',
		...
		switch ($timeFormat) {
			...
			break;
			case '12': // juan aqui cambie ':' por ' ' para cambiar estilo de select
